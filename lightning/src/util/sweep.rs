...
> where
	...
	O::Target: OutputSpender,
{
	sweeper: Arc<OutputSweeper<B, D, E, F, Arc<KVStoreSyncWrapper<K>>, L, O>>,
}

impl<B: Deref, D: Deref, E: Deref, F: Deref, K: Deref, L: Deref, O: Deref>
	OutputSweeperSyncKVStore<B, D, E, F, K, L, O>
where
	B::Target: BroadcasterInterface,
	D::Target: ChangeDestinationSource,
	E::Target: FeeEstimator,
	...
	O::Target: OutputSpender,
{
	/// Constructs a new [`OutputSweeperSyncKVStore`] instance.
	pub fn new(
		best_block: BestBlock, broadcaster: B, fee_estimator: E, chain_data_source: Option<F>,
		output_spender: O, change_destination_source: D, kv_store: K, logger: L,
	) -> Self {
	...
	pub async fn regenerate_and_broadcast_spend_if_necessary(&self) -> Result<(), ()> {
		self.sweeper.regenerate_and_broadcast_spend_if_necessary().await
	}
	...
	/// Wraps [`OutputSweeper::current_best_block`].
	pub fn current_best_block(&self) -> BestBlock {
	...
	pub fn sweeper_async(
		&self,
	) -> Arc<OutputSweeper<B, D, E, F, Arc<KVStoreSyncWrapper<K>>, L, O>> {
		Arc::clone(&self.sweeper)
	}
}

impl<B: Deref, D: Deref, E: Deref, F: Deref, K: Deref, L: Deref, O: Deref> Confirm
	for OutputSweeperSyncKVStore<B, D, E, F, K, L, O>
where
	B::Target: BroadcasterInterface,
	D::Target: ChangeDestinationSource,
	E::Target: FeeEstimator,
	...
	O::Target: OutputSpender,
{
	...
	fn block_disconnected(&self, header: &Header, height: u32) {
		self.sweeper.block_disconnected(header, height);
	}
}

impl<B: Deref, D: Deref, E: Deref, F: Deref, K: Deref, L: Deref, O: Deref>
	ReadableArgs<(B, E, Option<F>, O, D, K, L)> for OutputSweeperSyncKVStore<B, D, E, F, K, L, O>
where
	B::Target: BroadcasterInterface,
	D::Target: ChangeDestinationSource,
	E::Target: FeeEstimator,
	...
	O::Target: OutputSpender,
{
	fn read<R: io::Read>(
		...
		let kv_store = Arc::new(KVStoreSyncWrapper(args.5));

		let args = (args.0, args.1, args.2, args.3, args.4, kv_store, args.6);

		let sweeper = OutputSweeper::read(reader, args)?;

		Ok(Self { sweeper: Arc::new(sweeper) })
	}
}
