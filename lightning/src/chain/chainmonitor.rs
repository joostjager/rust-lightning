	...
	P::Target: Persist<ChannelSigner>,
{
	...
	///
	/// The consistent lock order is `pending_ops` **before** `monitors`.
	///
	/// [`watch_channel`] holds `pending_ops.lock()` **then** `monitors.read()` to atomically
	/// check for duplicate channel IDs in both the pending queue and the flushed set.
	///
	/// [`flush`] holds `pending_ops.lock()` across [`watch_channel_internal`] (which acquires
	/// `monitors.write()`) for [`NewMonitor`] ops, ensuring mutual exclusion with
	/// `watch_channel`: both acquire `pending_ops` first, so they serialize on that lock.
	///
	/// For [`Update`] ops, `pending_ops` is released before calling
	/// [`update_channel_internal`] so that concurrent `update_channel` queuing is not blocked.
	///
	/// [`watch_channel`]: chain::Watch::watch_channel
	/// [`flush`]: Self::flush
	/// [`watch_channel_internal`]: Self::watch_channel_internal
	/// [`update_channel_internal`]: Self::update_channel_internal
	...
	P::Target: Persist<ChannelSigner>,
{
	...
	///
	/// Returns `Ok(())` when all `count` operations (or the entire queue, if shorter) were
	/// flushed. Returns `Err(())` if a `NewMonitor` operation failed, in which case
...
mod tests {
	...
	fn test_queue_and_flush() {
		...
		deferred.flush(2, &&logger).unwrap();
		...
		deferred.flush(1, &&logger).unwrap();
		assert_eq!(deferred.pending_operation_count(), 0);
		...
		deferred.flush(5, &&logger).unwrap();
