...
pub(super) fn encrypt_failure_packet(shared_secret: &[u8], packet: &mut OnionErrorPacket) {
...
pub(super) fn build_first_hop_failure_packet(
	...
	let mut failure_packet =
		build_failure_packet(shared_secret, failure_type, failure_data, &payload);
...
fn decrypt_onion_error_packet(packet: &mut OnionErrorPacket, shared_secret: SharedSecret) {
	// Decrypt the packet.
...
mod tests {
	...
	fn test_unreadable_failure_packet_onion() {
		...
		encrypt_failure_packet(shared_secret, &mut onion_error_packet);
	...
	fn test_attributable_failure_packet_onion_mutations() {
		for mutating_node in 0..5 {
			for mutated_index in 0..1060 + 920 {
				...
				assert!(
					decrypted_failure.onion_error_code == Some(16399)
	...
	fn test_attributable_failure_packet_onion_with_mutation(
		...
		let onion_keys = build_test_onion_keys();
		let mut onion_error = super::build_failure_packet(
		...
		for idx in 1..5 {
			...
			if !mutated {
				...
				assert_eq!(
					onion_error.attribution_data.unwrap().to_lower_hex_string(),
					EXPECTED_MESSAGES[idx].attribution_data
				);
			}
		}
