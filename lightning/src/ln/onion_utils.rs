		...
		match self.0 {
			HTLCFailReasonRepr::Reason { ref failure_code, ref data } => {
				...
				if let Some(phantom_ss) = phantom_shared_secret {
					...
					let mut packet =
						build_failure_packet(phantom_ss, *failure_code, &data[..], &phantom_payload);
					encrypt_failure_packet(phantom_ss, &mut packet);
...
pub(crate) fn add_hmacs(shared_secret: &[u8], message: &[u8], packet: &mut [u8; ATTRIBUTION_DATA_LEN]) {
...
use crate::util::test_utils::TestLogger;
	...
	fn test_attributable_failure_packet_onion_mutations() {
		for mutating_node in 0..5 {
			for mutated_index in 0..1060+920 {
				...
				let decrypted_failure = test_attributable_failure_packet_onion_with_mutation(Some(mutation));
	...
	fn test_attributable_failure_packet_onion_with_mutation(mutation: Option<Mutation>) -> DecodedOnionFailure {
		...
		assert_eq!(onion_error.attribution_data.unwrap().to_lower_hex_string(), EXPECTED_MESSAGES[0].attribution_data);
		...
		for idx in 1..5 {
			...
			if !mutated {
				assert_eq!(onion_error.data.to_lower_hex_string(), EXPECTED_MESSAGES[idx].message);
				assert_eq!(onion_error.attribution_data.unwrap().to_lower_hex_string(), EXPECTED_MESSAGES[idx].attribution_data);
			}
		}
