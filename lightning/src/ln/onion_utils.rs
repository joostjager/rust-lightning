...
pub(crate) fn gen_rho_from_shared_secret(shared_secret: &[u8]) -> [u8; 32] {
	...
	let mut hmac = HmacEngine::<Sha256>::new(b"rho"); // rho
...
fn build_unencrypted_failure_packet(
	...
	let mut packet = OnionErrorPacket { data, attribution_data: Some([0; ATTRIBUTION_DATA_LEN]) };
...
fn update_attribution_data(
	...
	let attribution_data = onion_error_packet.attribution_data.as_mut().unwrap();
	let hold_time_bytes: [u8; 4] = hold_time.to_be_bytes();
	attribution_data[..HOLD_TIME_LEN].copy_from_slice(&hold_time_bytes);

	add_hmacs(shared_secret, &onion_error_packet.data, attribution_data);
}
	...
	L::Target: Logger,
{
	...
	while let Some((route_hop_idx, (route_hop_option, shared_secret))) = iterator.next() {
		...
		if let Some(ref mut attribution_data) = encrypted_packet.attribution_data {
			...
			let hold_time = verify_attribution_data(
				attribution_data,
				&encrypted_packet.data,
			...
			if let Some(hold_time) = hold_time {
				...
				shift_attribution_data_left(attribution_data);
			} else {
				res = Some(FailureLearnings {
					network_update: None,
...
pub(crate) fn add_hmacs(
	...
	let (hold_times, hmacs) = packet.split_at_mut(MAX_HOPS * HOLD_TIME_LEN);
	...
	for hmac_idx in 0..MAX_HOPS {
		...
		hmac_engine.input(&hold_times[..(position + 1) * HOLD_TIME_LEN]);
		write_downstream_hmacs(position, hmacs, &mut hmac_engine);
		...
		// Store the new hmac.
		hmacs[hmac_idx * HMAC_LEN..(hmac_idx + 1) * HMAC_LEN].copy_from_slice(hmac);
	}
}
...
pub fn write_downstream_hmacs(position: usize, hmacs: &[u8], w: &mut HmacEngine<Sha256>) {
...
fn process_failure_packet(
	...
	// Process received attribution data. If there's none, we'll still add our hold times and hmacs to potentially give
	// the sender attribution data for the partial path. In order for this to work, all upstream nodes need to support
	// attributable failures.
	if let Some(ref mut attribution_data) = onion_error.attribution_data {
		shift_attribution_data_right(attribution_data);
	} else {
		onion_error.attribution_data = Some([0u8; ATTRIBUTION_DATA_LEN]);
	}
...
fn shift_attribution_data_left(attribution_data: &mut [u8; ATTRIBUTION_DATA_LEN]) {
	// Shift hold times left.
	let hold_times = &mut attribution_data[..MAX_HOPS * HOLD_TIME_LEN];
	hold_times.copy_within(HOLD_TIME_LEN.., 0);

	// Shift hmacs left.
	let hmacs = &mut attribution_data[MAX_HOPS * HOLD_TIME_LEN..];
...
fn shift_attribution_data_right(attribution_data: &mut [u8; ATTRIBUTION_DATA_LEN]) {
	// Shift hold times right.
	{
		let hold_times = &mut attribution_data[..MAX_HOPS * HOLD_TIME_LEN];
		hold_times.copy_within(..hold_times.len() - HOLD_TIME_LEN, HOLD_TIME_LEN);
	}
...
fn verify_attribution_data(
	attribution_data: &[u8; ATTRIBUTION_DATA_LEN], message: &Vec<u8>, shared_secret: &[u8],
	position: usize,
) -> Option<u32> {
	let hold_times = &attribution_data[..MAX_HOPS * HOLD_TIME_LEN];
	let hmacs = &attribution_data[MAX_HOPS * HOLD_TIME_LEN..];

	// Calculate the expected HMAC.
	let um = gen_um_from_shared_secret(shared_secret);
	let mut hmac = HmacEngine::<Sha256>::new(&um);
	hmac.input(&message);
	hmac.input(&hold_times[..(position + 1) * HOLD_TIME_LEN]);
	write_downstream_hmacs(position, hmacs, &mut hmac);
	...
	let actual_hmac = &hmacs[hmac_idx * HMAC_LEN..hmac_idx * HMAC_LEN + HMAC_LEN];
	if !fixed_time_eq(expected_hmac, actual_hmac) {
		return None;
	}

	// The HMAC checks out and the hold time can be extracted and returned;
	let hold_time: u32 = u32::from_be_bytes(hold_times[..HOLD_TIME_LEN].try_into().unwrap());
