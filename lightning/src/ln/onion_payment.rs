...
pub(super) fn create_fwd_pending_htlc_info(
	...
	let (
		...
		onion_utils::Hop::Forward {
			next_hop_data:
				msgs::InboundOnionForwardPayload {
					short_channel_id,
					amt_to_forward,
					outgoing_cltv_value,
				},
		...
		onion_utils::Hop::TrampolineBlindedForward {
			...
			let (amt_to_forward, outgoing_cltv_value) = check_blinded_forward(
				msg.amount_msat,
				msg.cltv_expiry,
				&next_trampoline_hop_data.payment_relay,
				&next_trampoline_hop_data.payment_constraints,
				&next_trampoline_hop_data.features,
			)
			.map_err(|()| {
				// We should be returning malformed here if `msg.blinding_point` is set, but this is
				// unreachable right now since we checked it in `decode_update_add_htlc_onion`.
...
pub(super) fn create_recv_pending_htlc_info(
	hop_data: onion_utils::Hop, shared_secret: [u8; 32], payment_hash: PaymentHash, amt_msat: u64,
	cltv_expiry: u32, phantom_shared_secret: Option<[u8; 32]>, allow_underpay: bool,
	...
	let (
		...
		onion_utils::Hop::Receive {
			...
			None,
			false,
			keysend_preimage.is_none(),
			None,
		),
		onion_utils::Hop::BlindedReceive {
			hop_data:
				msgs::InboundOnionBlindedReceivePayload {
					sender_intended_htlc_amt_msat,
					total_msat,
					cltv_expiry_height,
					payment_secret,
	...
	L::Target: Logger,
{
	...
	Ok(match hop {
		...
		| onion_utils::Hop::BlindedForward { shared_secret, .. } => {
			...
			if let Err((err_msg, reason)) =
				check_incoming_htlc_cltv(cur_height, outgoing_cltv_value, msg.cltv_expiry)
			{
				return Err(InboundHTLCErr { msg: err_msg, reason, err_data: Vec::new() });
			}
	...
	L::Target: Logger,
{
	let encode_malformed_error = |message: &str, failure_reason: LocalHTLCFailureReason| {
		...
		let (sha256_of_onion, failure_reason) = if msg.blinding_point.is_some()
			|| failure_reason == LocalHTLCFailureReason::InvalidOnionBlinding
		{
	...
	let encode_relay_error = |message: &str,
	...
	let next_hop = match onion_utils::decode_next_payment_hop(
		...
		Err(onion_utils::OnionDecodeErr::Relay {
			...
			return encode_relay_error(
				err_msg,
				reason,
				shared_secret.secret_bytes(),
				trampoline_shared_secret.map(|tss| tss.secret_bytes()),
				&[0; 0],
			);
		},
	};
	...
	let next_packet_details = match next_hop {
		...
		onion_utils::Hop::TrampolineForward {
			next_trampoline_hop_data:
				msgs::InboundTrampolineForwardPayload {
					amt_to_forward,
					outgoing_cltv_value,
					next_trampoline,
				},
			...
			let next_trampoline_packet_pubkey = onion_utils::next_hop_pubkey(
				secp_ctx,
				incoming_trampoline_public_key,
				&trampoline_shared_secret.secret_bytes(),
			);
			Some(NextPacketDetails {
