...
fn do_mpp_receive_timeout(send_partial_mpp: bool) {
	...
	let path = &[&nodes[1], &nodes[3]];
	pass_along_path(&nodes[0], path, 200_000, hash, Some(pay_secret), node_1_msgs, false, None);
	...
	if send_partial_mpp {
		...
		let path = &[&nodes[2], &nodes[3]];
		let secret = Some(pay_secret);
		pass_along_path(&nodes[0], path, 200_000, hash, secret, node_2_msgs, true, None);
...
fn test_mpp_keysend() {
	...
	let preimage = Some(PaymentPreimage([42; 32]));
	let secret = PaymentSecret([42; 32]);
	...
	pass_along_path(&nodes[0], route[0], recv_value, hash, Some(secret), ev, false, preimage);

	let ev = remove_first_msg_event_to_node(&node_c_id, &mut events);
	pass_along_path(&nodes[0], route[1], recv_value, hash, Some(secret), ev, true, preimage);
...
fn do_retry_with_no_persist(confirm_before_reload: bool) {
	...
	// out and retry.
	let amt_msat = 1_000_000;
	let (route, payment_hash, payment_preimage, secret) =
	...
	let htlc_fulfill = get_htlc_update_msgs!(nodes[2], node_b_id);
	nodes[1].node.handle_update_fulfill_htlc(node_c_id, &htlc_fulfill.update_fulfill_htlcs[0]);
	check_added_monitors!(nodes[1], 1);
	...
	pass_along_path(&nodes[0], path, 1_000_000, payment_hash, Some(secret), event, true, None);
...
fn do_test_completed_payment_not_retryable_on_reload(use_dust: bool) {
	...
	let config = test_default_channel_config();
	reload_node!(nodes[0], config, node_a_ser, &[&mon_ser], persist_1, chain_1, node_a_reload_1);
	nodes[1].node.peer_disconnected(node_a_id);
	...
	reload_node!(nodes[0], config, node_a_ser, monitors, persist_2, chain_2, node_a_reload_2);
	nodes[1].node.peer_disconnected(node_a_id);
	...
	let config = test_default_channel_config();
	let monitors = &[&chan_0_monitor_serialized[..], &chan_1_monitor_serialized[..]];
	reload_node!(nodes[0], config, node_a_ser, monitors, persist_3, chain_3, node_a_reload_3);
	nodes[1].node.peer_disconnected(node_a_id);
...
fn do_test_dup_htlc_onchain_doesnt_fail_on_reload(
	...
	// Now reload nodes[0]...
	reload_node!(nodes[0], &chan_manager_serialized, &[&mon_ser], persister, chain, node_a_reload);
...
fn test_fulfill_restart_failure() {
	...
	let htlc_fulfill = get_htlc_update_msgs!(nodes[1], node_a_id);
	nodes[0].node.handle_update_fulfill_htlc(node_b_id, &htlc_fulfill.update_fulfill_htlcs[0]);
	expect_payment_sent(&nodes[0], payment_preimage, None, false, false);

	// Now reload nodes[1]...
	reload_node!(nodes[1], &chan_manager_serialized, &[&mon_ser], persister, chain, node_b_reload);
...
fn auto_retry_partial_failure() {
	...
	nodes[0].node.handle_update_fulfill_htlc(node_b_id, &bs_claim.update_fulfill_htlcs[0]);
	expect_payment_sent(&nodes[0], payment_preimage, None, false, false);
	...
	let bs_2nd_claim = get_htlc_update_msgs!(nodes[1], node_a_id);
	...
	nodes[0].node.handle_update_fulfill_htlc(node_b_id, &bs_2nd_claim.update_fulfill_htlcs[0]);
	nodes[0].node.handle_update_fulfill_htlc(node_b_id, &bs_2nd_claim.update_fulfill_htlcs[1]);
...
fn do_no_missing_sent_on_reload(persist_manager_with_payment: bool, at_midpoint: bool) {
	...
	if at_midpoint {
		...
		let htlc_fulfill = get_htlc_update_msgs!(nodes[1], node_a_id);
		nodes[0].node.handle_update_fulfill_htlc(node_b_id, &htlc_fulfill.update_fulfill_htlcs[0]);
		commitment_signed_dance!(nodes[0], nodes[1], htlc_fulfill.commitment_signed, false);
...
fn do_claim_from_closed_chan(fail_payment: bool) {
	...
	let (payment_preimage, hash, secret) = get_payment_preimage_hash!(nodes[3]);
	let payment_params = PaymentParameters::from_node_id(node_d_id, TEST_FINAL_CLTV)
		.with_bolt11_features(nodes[1].node.bolt11_invoice_features())
		.unwrap();
	...
	pass_along_path(&nodes[0], path_a, 10_000_000, hash, Some(secret), msg_a, false, None);
	let receive_event =
		pass_along_path(&nodes[0], path_b, 10_000_000, hash, Some(secret), msg_b, true, None);
...
fn do_test_custom_tlvs(spontaneous: bool, even_tlvs: bool, known_tlvs: bool) {
	...
	let amt_msat = 100_000;
	let (mut route, hash, preimage, secret) =
		get_route_and_payment_hash!(&nodes[0], &nodes[1], amt_msat);
	let id = PaymentId(hash.0);
	let custom_tlvs = vec![
	...
	let onion = RecipientOnionFields {
		payment_secret: if spontaneous { None } else { Some(secret) },
...
fn do_test_custom_tlvs_consistency(
	...
	pass_along_path(&nodes[0], path_a, amt_msat, hash, Some(secret), event, false, None);
...
fn pay_route_without_params() {
	...
	assert_eq!(events.len(), 1);
	let node_1_msgs = remove_first_msg_event_to_node(&node_b_id, &mut events);
	pass_along_path(&nodes[0], &[&nodes[1]], amt_msat, hash, Some(secret), node_1_msgs, true, None);
	claim_payment_along_route(ClaimAlongRouteArgs::new(&nodes[0], &[&[&nodes[1]]], preimage));
}
