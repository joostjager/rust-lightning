...
fn failed_probe_yields_event() {
	...
	let probe_event =
		SendEvent::from_commitment_update(nodes[1].node.get_our_node_id(), channel_id, updates);
...
fn onchain_failed_probe_yields_event() {
	...
	let updates = get_htlc_update_msgs!(nodes[0], nodes[1].node.get_our_node_id());
	let probe_event =
		SendEvent::from_commitment_update(nodes[1].node.get_our_node_id(), chan_id, updates);
	nodes[1].node.handle_update_add_htlc(nodes[0].node.get_our_node_id(), &probe_event.msgs[0]);
	check_added_monitors!(nodes[1], 0);
	commitment_signed_dance!(nodes[1], nodes[0], probe_event.commitment_msg, false);
	expect_pending_htlcs_forwardable!(nodes[1]);
...
fn auto_retry_partial_failure() {
	...
	nodes[1]
		.node
		.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &as_third_cs);
	check_added_monitors!(nodes[1], 1);
	...
	nodes[1]
		.node
		.handle_commitment_signed_batch_test(nodes[0].node.get_our_node_id(), &as_fourth_cs);
	check_added_monitors!(nodes[1], 1);
...
fn no_extra_retries_on_back_to_back_fail() {
	...
	nodes[1]
		.node
		.handle_update_add_htlc(nodes[0].node.get_our_node_id(), &first_htlc_updates.msgs[0]);
	nodes[1].node.handle_commitment_signed_batch_test(
		nodes[0].node.get_our_node_id(),
		&first_htlc_updates.commitment_msg,
	);
	check_added_monitors!(nodes[1], 1);
	...
	nodes[0].node.handle_update_fail_htlc(
		nodes[1].node.get_our_node_id(),
		&bs_fail_update.update_fail_htlcs[1],
	);
...
fn test_simple_partial_retry() {
	...
	nodes[2].node.handle_update_add_htlc(
		nodes[1].node.get_our_node_id(),
		&bs_second_forward_update.update_add_htlcs[0],
	);
	commitment_signed_dance!(
		nodes[2],
		nodes[1],
