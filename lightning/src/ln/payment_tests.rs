...
fn mpp_retry() {
	...
	let success_path_msgs = remove_first_msg_event_to_node(&node_b_id, &mut events);
	pass_along_path(
		&nodes[0],
		&[&nodes[1], &nodes[3]],
		2_000_000,
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(
		&nodes[2],
		vec![HTLCHandlingFailureType::Forward { node_id: Some(node_d_id), channel_id: chan_4_id }]
	);
...
fn mpp_retry_overpay() {
	...
	let node_chanmgrs = create_node_chanmgrs(
		...
		&[
			...
			Some(limited_config_1),
			Some(limited_config_2),
			Some(user_config),
		],
	);
	let nodes = create_network(4, &node_cfgs, &node_chanmgrs);
...
fn do_mpp_receive_timeout(send_partial_mpp: bool) {
	...
	if send_partial_mpp {
		...
		let htlc_fail_updates_3_1 = get_htlc_update_msgs!(nodes[3], node_b_id);
		assert_eq!(htlc_fail_updates_3_1.update_fail_htlcs.len(), 1);
		nodes[1]
			.node
			.handle_update_fail_htlc(node_d_id, &htlc_fail_updates_3_1.update_fail_htlcs[0]);
		...
		let htlc_fail_updates_1_0 = get_htlc_update_msgs!(nodes[1], node_a_id);
		assert_eq!(htlc_fail_updates_1_0.update_fail_htlcs.len(), 1);
		nodes[0]
			.node
			.handle_update_fail_htlc(node_b_id, &htlc_fail_updates_1_0.update_fail_htlcs[0]);
...
fn do_retry_with_no_persist(confirm_before_reload: bool) {
	...
	nodes[1]
		.node
		.handle_update_fulfill_htlc(node_c_id, &htlc_fulfill_updates.update_fulfill_htlcs[0]);
	check_added_monitors!(nodes[1], 1);
...
fn test_fulfill_restart_failure() {
	...
	let htlc_fulfill_updates = get_htlc_update_msgs!(nodes[1], node_a_id);
	nodes[0]
		.node
		.handle_update_fulfill_htlc(node_b_id, &htlc_fulfill_updates.update_fulfill_htlcs[0]);
...
fn do_test_intercepted_payment(test: InterceptTest) {
	...
	if test == InterceptTest::Fail {
		...
		let temp_chan_id =
			nodes[1].node.create_channel(node_c_id, 100_000, 0, 42, None, None).unwrap();
		...
		nodes[0]
			.node
			.handle_update_fail_htlc(node_b_id, &htlc_timeout_updates.update_fail_htlcs[0]);
...
fn do_accept_underpaying_htlcs_config(num_mpp_parts: usize) {
	...
	for _ in 0..num_mpp_parts {
		...
		let channel_id =
			create_unannounced_chan_between_nodes_with_value(&nodes, 1, 2, channel_size, 0)
				.0
				.channel_id;
...
fn auto_retry_partial_failure() {
	...
	nodes[1].node.handle_update_add_htlc(node_a_id, &as_second_htlc_updates.msgs[0]);
	nodes[1].node.handle_update_add_htlc(node_a_id, &as_second_htlc_updates.msgs[1]);
	nodes[1]
		.node
		.handle_commitment_signed_batch_test(node_a_id, &as_second_htlc_updates.commitment_msg);
	check_added_monitors!(nodes[1], 1);
	...
	let bs_claim_update = get_htlc_update_msgs!(nodes[1], node_a_id);
	assert_eq!(bs_claim_update.update_fulfill_htlcs.len(), 1);

	nodes[0].node.handle_update_fulfill_htlc(node_b_id, &bs_claim_update.update_fulfill_htlcs[0]);
	expect_payment_sent(&nodes[0], payment_preimage, None, false, false);
	nodes[0]
		.node
		.handle_commitment_signed_batch_test(node_b_id, &bs_claim_update.commitment_signed);
	check_added_monitors!(nodes[0], 1);
	...
	let bs_second_claim_update = get_htlc_update_msgs!(nodes[1], node_a_id);
	...
	nodes[0]
		.node
		.handle_update_fulfill_htlc(node_b_id, &bs_second_claim_update.update_fulfill_htlcs[0]);
	nodes[0]
		.node
		.handle_update_fulfill_htlc(node_b_id, &bs_second_claim_update.update_fulfill_htlcs[1]);
	nodes[0]
		.node
		.handle_commitment_signed_batch_test(node_b_id, &bs_second_claim_update.commitment_signed);
	check_added_monitors!(nodes[0], 1);
...
fn no_extra_retries_on_back_to_back_fail() {
	...
	nodes[1].node.handle_update_add_htlc(node_a_id, &first_htlc_updates.msgs[0]);
	nodes[1]
		.node
		.handle_commitment_signed_batch_test(node_a_id, &first_htlc_updates.commitment_msg);
	check_added_monitors!(nodes[1], 1);
	...
	nodes[1]
		.node
		.handle_commitment_signed_batch_test(node_a_id, &second_htlc_updates.commitment_msg);
	check_added_monitors!(nodes[1], 1);
...
fn test_simple_partial_retry() {
	...
	let bs_second_forward_update = get_htlc_update_msgs!(nodes[1], node_c_id);
	nodes[2].node.handle_update_add_htlc(node_b_id, &bs_second_forward_update.update_add_htlcs[0]);
	commitment_signed_dance!(
		nodes[2],
		nodes[1],
		&bs_second_forward_update.commitment_signed,
		false
	);
...
fn do_claim_from_closed_chan(fail_payment: bool) {
	...
	let mut route = nodes[0]
		.router
		.find_route(&node_a_id, &route_params, None, nodes[0].node.compute_inflight_htlcs())
		.unwrap();
	...
	if fail_payment {
		...
		let reason = HTLCHandlingFailureType::Receive { payment_hash };
		...
		nodes[1]
			.node
			.force_close_broadcasting_latest_txn(&chan_bd, &node_d_id, error_message.to_string())
			.unwrap();
		check_closed_event!(
			&nodes[1],
			1,
		...
		{
			nodes[2].node.handle_update_fulfill_htlc(node_d_id, &updates.update_fulfill_htlcs[0]);
		...
		if let MessageSendEvent::UpdateHTLCs { updates, .. } = &cs_claim_msgs[0] {
			nodes[0].node.handle_update_fulfill_htlc(node_c_id, &updates.update_fulfill_htlcs[0]);
...
fn test_retry_custom_tlvs() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(
		&nodes[1],
		vec![HTLCHandlingFailureType::Forward { node_id: Some(node_c_id), channel_id: chan_2_id }]
	);
	check_added_monitors!(nodes[1], 1);
	...
	let args =
		PassAlongPathArgs::new(&nodes[0], path, 1_000_000, payment_hash, events.pop().unwrap())
...
fn do_test_payment_metadata_consistency(do_reload: bool, do_modify: bool) {
	...
	let node_chanmgrs = create_node_chanmgrs(
		4,
		&node_cfgs,
		&[None, Some(config.clone()), Some(config.clone()), Some(config.clone())],
	);
	let nodes_0_deserialized;
...
fn test_htlc_forward_considers_anchor_outputs_value() {
	...
	expect_pending_htlcs_forwardable_and_htlc_handling_failed!(
		&nodes[1],
		vec![HTLCHandlingFailureType::Forward { node_id: Some(node_c_id), channel_id: chan_id_2 }]
	);
	check_added_monitors(&nodes[1], 1);
...
fn test_non_strict_forwarding() {
	...
	for i in 0..4 {
		...
		assert_eq!(send_event.msgs[0].channel_id, if i < 2 { channel_id_1 } else { channel_id_2 });
	...
	expect_payment_failed_conditions_event(
		...
		payment_hash,
		false,
		PaymentFailedConditions::new().blamed_scid(routed_scid),
	);
}
...
fn max_out_mpp_path() {
	...
	let node_chanmgrs = create_node_chanmgrs(
		3,
		&node_cfgs,
		&[Some(user_cfg.clone()), Some(lsp_cfg.clone()), Some(user_cfg.clone())],
	);
	let nodes = create_network(3, &node_cfgs, &node_chanmgrs);
