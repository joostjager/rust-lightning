...
fn archive_fully_resolved_monitors() {
	...
	let node_chanmgrs =
		create_node_chanmgrs(2, &node_cfgs, &[Some(user_config.clone()), Some(user_config)]);
	...
	if let Event::SpendableOutputs { .. } = spendable_event[0] {
	} else {
		panic!();
	}
...
fn do_test_claim_value_force_close(anchors: bool, prev_commitment_tx: bool) {
	...
	assert_ne!(
		...
		a_broadcast_txn[0].input[1].previous_output.vout
	);
	// a_broadcast_txn [0] and [1] should spend the HTLC outputs of the commitment tx
	assert!(a_broadcast_txn[0].input.iter().any(|input| remote_txn[0].output
...
fn do_test_balances_on_local_commitment_htlcs(anchors: bool) {
	...
	if anchors {
		...
		assert_eq!(
			timeout_htlc_txn[0].input[0].witness.last().unwrap().len(),
			chan_utils::OFFERED_HTLC_SCRIPT_WEIGHT_ANCHORS
		);
		assert_eq!(
			timeout_htlc_txn[0].input[1].witness.last().unwrap().len(),
			chan_utils::OFFERED_HTLC_SCRIPT_WEIGHT_ANCHORS
		);
	} else {
		...
		assert_eq!(
			timeout_htlc_txn[0].input[0].witness.last().unwrap().len(),
			chan_utils::OFFERED_HTLC_SCRIPT_WEIGHT
		);
		assert_eq!(
			timeout_htlc_txn[1].input[0].witness.last().unwrap().len(),
			chan_utils::OFFERED_HTLC_SCRIPT_WEIGHT
		);
	}
...
fn do_test_revoked_counterparty_commitment_balances(anchors: bool, confirm_htlc_spend_first: bool) {
	...
	events.retain(|ev| match ev {
		Event::HTLCHandlingFailed {
			failure_type: HTLCHandlingFailureType::Forward { node_id, channel_id },
			..
		} => {
		...
		Event::HTLCHandlingFailed {
			failure_type: HTLCHandlingFailureType::Receive { payment_hash },
			..
		} => {
	...
	let pinnable_fee = claim_txn[0]
		...
		.sum::<u64>()
		- claim_txn[0].output.iter().map(|txout| txout.value.to_sat()).sum::<u64>();
	let unpinnable_fee = claim_txn[1]
		.input
		.iter()
		.map(|txin| {
			assert_eq!(txin.previous_output.txid, as_revoked_txn[0].compute_txid());
		...
		.sum::<u64>()
		- claim_txn[1].output.iter().map(|txout| txout.value.to_sat()).sum::<u64>();
	...
	let htlc_unclaimed_balance =
		|amount: u64| Balance::CounterpartyRevokedOutputClaimable { amount_satoshis: amount };
...
fn do_test_revoked_counterparty_htlc_tx_balances(anchors: bool) {
	...
	let revoked_to_self_claim = {
		...
		assert_ne!(
			as_commitment_claim_txn[0].input[0].previous_output,
			as_commitment_claim_txn[1].input[0].previous_output
		);
		assert_ne!(
			as_commitment_claim_txn[0].input[0].previous_output,
			as_commitment_claim_txn[1].input[1].previous_output
		);
		assert_ne!(
			as_commitment_claim_txn[1].input[0].previous_output,
			as_commitment_claim_txn[1].input[1].previous_output
...
fn do_test_revoked_counterparty_aggregated_claims(anchors: bool) {
	...
	let spendable_output_events =
		nodes[1].chain_monitor.chain_monitor.get_and_clear_pending_events();
...
fn do_test_claimable_balance_correct_while_payment_pending(outbound_payment: bool, anchors: bool) {
	...
	let node_chanmgrs = create_node_chanmgrs(
		3,
		&node_cfgs,
		&[Some(user_config.clone()), Some(user_config.clone()), Some(user_config)],
	);
...
fn test_anchors_aggregated_revoked_htlc_tx() {
	...
	*nodes[1].fee_estimator.sat_per_kw.lock().unwrap() *= 2;
	...
	// clusters: the unpinnable, revoked to_self outputs and the pinnable, revoked HTLC outputs.
	{
		...
		let revoked_claims_a: Vec<_> = txn
			.clone()
			.into_iter()
			.filter(|tx| {
			...
			.collect();
		let revoked_claims_b: Vec<_> = txn
			.clone()
			.into_iter()
			.filter(|tx| {
	...
	let htlc_tx = {
		...
		for (idx, htlc_descriptor) in descriptors.into_iter().enumerate() {
			...
			let signer = nodes[1]
				.keys_manager
				.derive_channel_signer(htlc_descriptor.channel_derivation_parameters.keys_id);
			let our_sig = signer
...
fn test_update_replay_panics() {
	...
	std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
		let _ = poisoned_monitor.update_monitor(
			&updates[1],
			&nodes[1].tx_broadcaster,
			&nodes[1].fee_estimator,
			&nodes[1].logger,
