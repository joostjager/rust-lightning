				...
				F3: FnMut(),
{
	...
	if update_1_0.update_fail_htlcs.len() > 0 {
		...
		// DEBUG write failure message in wire format.
		let writer = &mut VecWriter(Vec::new());
		fail_msg.write(writer).unwrap();
		eprintln!("fail_msg: {}", writer.0.to_lower_hex_string());
...
fn test_onion_failure() {
	...
	run_onion_failure_test("expiry_too_far", 100, &nodes, &route, &payment_hash, &payment_secret, |msg| {
		let session_priv = SecretKey::from_slice(&[3; 32]).unwrap();
	...
	run_onion_failure_test_with_fail_intercept("0-length channel update in intermediate node UPDATE onion failure",
		...
		Some(channels[1].0.contents.short_channel_id), Some(next_hop_failure.clone()));
...
#[test]
fn test_attributable_onion_failure() {
	// When we check for amount_below_minimum below, we want to test that we're using the *right*
	// amount, thus we need different htlc_minimum_msat values. We set node[2]'s htlc_minimum_msat
	// to 2000, which is above the default value of 1000 set in create_node_chanmgrs.
	// This exposed a previous bug because we were using the wrong value all the way down in
...
fn test_trampoline_onion_payload_assembly_values() {
	...
	let (trampoline_payloads, outer_total_msat, outer_starting_htlc_offset) = onion_utils::build_trampoline_onion_payloads(&path.blinded_tail.as_ref().unwrap(), amt_msat, &recipient_onion_fields, cur_height, &None).unwrap();
	...
	let onion_keys = construct_trampoline_onion_keys(&secp_ctx, &path.blinded_tail.as_ref().unwrap(), &session_priv).unwrap();
...
fn test_trampoline_onion_payload_construction_vectors() {
	...
	let trampoline_onion_keys = construct_trampoline_onion_keys(&Secp256k1::new(), &trampoline_hops.blinded_tail.unwrap(), &trampoline_session_key).unwrap();
